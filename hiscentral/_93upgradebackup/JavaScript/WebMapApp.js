...
var arcgisWebAppCurrentMode = "MapPan";
var arcgisWebAppLastMode = "MapPan";
...
function initializeApp() {
        ...
        if (newLoad) {
            ...
            var box = Sys.UI.DomElement.getLocation($get("Map_Panel")); 
            var mag = $get("Magnifier1");
            if (mag!=null) { Sys.UI.DomElement.setLocation(mag, box.x, box.y); }
            var tb = document.getElementById(m_measureToolbarId);
			if (tb!=null) {
			    tb.style.left = box.x + "px";
			    tb.style.top = box.y + "px";
			}
		 	var fp = $get("CopyrightText_Panel");
			if (fp!=null) { Sys.UI.DomElement.setLocation(fp,box.x, box.y); }
       ...
       for (var fp in FloatingPanels) {
            FloatingPanels[fp].add_docked(scrollDockToPanel);
       }  
        ...
        var taskResults = $find("Results_TaskResults1");
...
function OnToolSelectHandler(sender, args) {
    if (args.name) {
        var mode = args.name;
        arcgisWebAppCurrentMode = mode;
        if (mode!="Measure" && arcgisWebAppLastMode=="Measure") closeMeasureToolbarTool();
        if (mode!="MapIdentify" && arcgisWebAppLastMode=="MapIdentify") {
            if (arcgisIdentifyTool!=null) closeIdentifyPanel();
        } 
        arcgisWebAppLastMode = mode;
    }
}
